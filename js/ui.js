...
(function() {
    ...
    let statusDropdownListenersAdded = false;
    ...
    APP.configurarBusca = function() {
        ...
        const novoBuscaInput = buscaInput.cloneNode(true);
        buscaInput.parentNode.replaceChild(novoBuscaInput, buscaInput);

        novoBuscaInput.addEventListener('input', function() {
        ...
        if (buscaLimpar) {
            const novoBuscaLimpar = buscaLimpar.cloneNode(true);
            buscaLimpar.parentNode.replaceChild(novoBuscaLimpar, buscaLimpar);
            novoBuscaLimpar.addEventListener('click', function() {
                const input = document.getElementById('buscaPacientes');
                if (input) {
                    input.value = '';
                    termoBusca = '';
        ...
        novoBuscaInput.addEventListener('keydown', function(e) {
            if (e.key === 'Escape') {
                this.value = '';
                termoBusca = '';
    ...
    APP.popularStatusSelects = function() {
        ...
        const padrao = APP.STATUS_PADRAO || STATUS_KEYS[0] || 'espera';
    ...
    APP.getPacientesFiltrados = function() {
        ...
        if (localFiltro !== 'todos') {
            filtrados = filtrados.filter(p => p && p.local === localFiltro);
        }
        if (encFiltro !== 'todos') {
            filtrados = filtrados.filter(p => p && p.encaminhamentos && p.encaminhamentos.includes(encFiltro));
        }
        if (statusFiltro !== 'todos') {
            filtrados = filtrados.filter(p => p && p.status === statusFiltro);
        }
    ...
    APP.renderizarTabela = function() {
                ...
                <tr>
                    ...
                    <td>
                        <div class="acoes-cell">
                            ...
                            <button class="btn btn-outline" style="padding: 0.2rem 0.7rem; font-size: 0.75rem;" data-id="${p.id}">
        ...
        document.querySelectorAll('.btn-edit').forEach(el => {
            const novoEl = el.cloneNode(true);
            el.parentNode.replaceChild(novoEl, el);
            novoEl.addEventListener('click', function(e) {
                e.stopPropagation();
                const id = parseInt(this.dataset.id);
                if (!isNaN(id)) APP.editarPaciente(id);
            });
        });
        ...
        document.querySelectorAll('.acoes-cell .btn-outline').forEach(el => {
            ...
            novoEl.addEventListener('click', function(e) {
                e.stopPropagation();
                const id = parseInt(this.dataset.id);
                if (!isNaN(id)) APP.removerPaciente(id);
            });
        });

        // Eventos para status dropdown
    ...
    APP.popularSelects = function() {
        ...
        if (filtroLocal) {
            ...
            const todosOption = filtroLocal.querySelector('option[value="todos"]');
            filtroLocal.innerHTML = '';
            const optTodos = document.createElement('option');
            optTodos.value = 'todos';
            optTodos.textContent = 'Todos os locais';
            filtroLocal.appendChild(optTodos);
        ...
        if (filtroEncaminhamento) {
            const todosOption = filtroEncaminhamento.querySelector('option[value="todos"]');
            filtroEncaminhamento.innerHTML = '';
            const optTodos = document.createElement('option');
            optTodos.value = 'todos';
            optTodos.textContent = 'Todos';
            filtroEncaminhamento.appendChild(optTodos);
