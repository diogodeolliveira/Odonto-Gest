...
(function() {
    ...
    // Estado global usado pelo odontograma.js e pelos modais
    APP.pacientes = APP.pacientes || [];
    APP.dentesSelecionados = new Set();
    APP.encaminhamentosTemp = [];

    // ============================================================
    // INICIALIZAÇÃO — chamado no DOMContentLoaded lá embaixo.
    // Sem isso, NENHUM botão do sistema (fora o "Entrar") tinha
    // listener nenhum: era essa a causa dos botões "congelados".
    // ============================================================
    async function init() {
        ...
        // Restaura sessão salva (se houver) ao atualizar a página
    ...
    // ============================================================
    // BOTÕES PRINCIPAIS DO HEADER E FILTROS
    // ============================================================
    ...
    function abrirModalCadastro(paciente) {
        ...
        if (titulo) titulo.innerHTML = paciente ?
            '<i class="fas fa-user-md"></i> Editar Paciente' :
            '<i class="fas fa-user-md"></i> Novo Paciente';
        if (subtitulo) subtitulo.textContent = paciente ?
    ...
    // ---- Encaminhamentos (tags dentro do modal de cadastro) ----
    ...
    async function salvarPaciente() {
        ...
        try {
            ...
            if (editId) {
                ...
                // Se a policy de RLS não devolver a linha, usamos os dados
                // que acabamos de enviar em vez de gravar "undefined" no
                // array (isso corrompe o cache local — vira null no
                // localStorage e quebra a tabela depois).
                ...
                if (data && data[0]) {
                    ...
                    console.warn('⚠️ INSERT feito, mas Supabase não retornou a linha (verifique a policy de SELECT no RLS). Recarregando lista da nuvem...');
        ...
        input.addEventListener('change', function() {
            ...
            reader.onload = async function(e) {
                try {
                    ...
                    if (!confirm(`Importar ${validos.length} paciente(s)? Isso vai enviar os dados para a nuvem.`)) {
                        return;
                    }

                    // Envia sem o "id" para o Supabase gerar novos IDs e não
                    // colidir com pacientes já existentes.
