...
(function() {
    ...
    APP.configurarEventos = function() {
        ...
        // Menu "Mais opções" (três pontinhos)
        const btnMenuMais = document.getElementById('btnMenuMais');
        const menuMaisContent = document.getElementById('menuMaisContent');
        if (btnMenuMais && menuMaisContent) {
            btnMenuMais.addEventListener('click', function(e) {
                e.stopPropagation();
                const abrindo = !menuMaisContent.classList.contains('active');
                menuMaisContent.classList.toggle('active', abrindo);
                btnMenuMais.classList.toggle('active', abrindo);
                btnMenuMais.setAttribute('aria-expanded', abrindo ? 'true' : 'false');
            });

            // Fecha o menu ao clicar em qualquer opção dentro dele
            menuMaisContent.addEventListener('click', function(e) {
                if (e.target.closest('button')) {
                    menuMaisContent.classList.remove('active');
                    btnMenuMais.classList.remove('active');
                    btnMenuMais.setAttribute('aria-expanded', 'false');
                }
            });
