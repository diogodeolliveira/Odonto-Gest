...
(function() {
    ...
    APP.salvarPaciente = async function() {
        ...
        try {
            ...
            if (!session) {
                APP.mostrarToast('⚠️ Faça login para salvar', '#8a6a3a');
                return;
            }

            if (editId) {
                // ✅ CORRIGIDO: Verifica conflito antes de atualizar
                paciente.id = parseInt(editId);
                
                // Busca versão atual no banco
                ...
                // Verifica se alguém alterou enquanto o usuário editava
                if (atual && new Date(atual.updated_at) > new Date(paciente.updated_at)) {
                    if (!confirm('⚠️ Este paciente foi alterado por outra pessoa. Deseja sobrescrever?')) {
                        return;
                    }
                }
    ...
    APP.atualizarListaEncaminhamentos = function() {
        ...
        APP.encaminhamentosTemp.forEach((enc, index) => {
            ...
            tag.innerHTML = `${APP.escapeHTML(enc)} <i class="fas fa-times-circle" data-index="${index}"></i>`;
    ...
    APP.adicionarEncaminhamento = function() {
        ...
        if (APP.encaminhamentosTemp.includes(valor)) {
            APP.mostrarToast('Este encaminhamento já foi adicionado.', '#8a8a3a');
            return;
        }
    ...
    APP.removerPaciente = async function(id) {
        ...
        // ✅ CORRIGIDO: Usa modal de confirmação em vez de confirm()
    ...
    APP.gerarPDF = function() {
        ...
        if (filtrados.length === 0) {
            APP.mostrarToast('⚠️ Não há pacientes para gerar o relatório.', '#8a8a3a');
            return;
        }
        ...
        filtrados.forEach((p, index) => {
            ...
            html += `
                <tr>
                    ...
                    <td>${APP.escapeHTML(encaminhamentosStr || '—')}</td>
        ...
        html2canvas(element, { scale: 2, useCORS: true, logging: false, backgroundColor: '#ffffff' })
            ...
            .catch(err => {
                ...
                APP.mostrarToast('❌ Erro ao gerar PDF', '#7a3a3a');
            });
    };
            ...
            reader.onload = async function(ev) {
                try {
                    ...
                    if (Array.isArray(dados) && dados.length > 0) {
                        ...
                        APP.mostrarToast('⚠️ Arquivo inválido ou vazio.', '#8a8a3a');
                    }
                } catch (err) {
                    APP.mostrarToast('❌ Erro ao ler o arquivo. Certifique-se de que é um JSON válido.', '#7a3a3a');
                }
            };
