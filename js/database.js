...
(function() {
    ...
    APP.salvarDadosLocal = function() {
        try {
            ...
            const sanitizados = APP.sanitizarPacientes(dados);
            localStorage.setItem(CONFIG.STORAGE_KEY, JSON.stringify(sanitizados));
            localStorage.setItem(CONFIG.STORAGE_KEY + '_backup', JSON.stringify(sanitizados));
        } catch (e) {
    ...
    APP.enviarParaSupabase = async function(dados) {
        try {
            ...
            // Sanitiza dados
            const dadosSanitizados = APP.sanitizarPacientes(dados);
            if (dadosSanitizados.length === 0) {
                APP.mostrarToast('⚠️ Nenhum dado válido para enviar', '#8a6a3a');
                return false;
            }

            const dadosComTimestamp = dadosSanitizados.map(p => ({
                ...p,
            ...
            APP.pacientes = dadosSanitizados;
            APP.salvarDadosLocal();

            APP.mostrarToast(`📤 ${dadosSanitizados.length} pacientes sincronizados!`, '#1a6a4a');
            return true;
        } catch (error) {
    ...
    APP.sincronizar = async function() {
        ...
        try {
            ...
            if (!conectado) {
                ...
                APP.mostrarToast('⚠️ Sem conexão com a nuvem', '#8a6a3a');
                return;
            }
            ...
            const merged = local.map(p => {
                ...
                if (nuvemItem) {
                    const dataLocal = p.updated_at ? new Date(p.updated_at) : new Date(0);
                    const dataNuvem = nuvemItem.updated_at ? new Date(nuvemItem.updated_at) : new Date(0);
            ...
            // Compara os dados
            const mergedStr = JSON.stringify(mergedOrdenado);
            const localStr = JSON.stringify(localOrdenado);
